...
{\bfseries ANSWER: }We know that $lg^*(lgn)=1+lg^*(lgn)$, so $lg^*(lgn)=lg^*(lgn)-1$. Therefore, we have $\frac{f(n)}{g(n)}= \frac{lg^*(lgn)}{lg^*(lgn)-1}$. We can let $x=lg^*(n)$, giving us $\frac{lg(x)}{x-1}$. As x goes to infinity, $g(n)$ grows a lot faster than $g(n)$ does, so $f(n) = O(g)$.
